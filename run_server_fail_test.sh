...
NUM_CLIENTS=10
TRIAL=1
FAILURE_DELAY=1.5      # seconds after clients start before killing server
...
for STRATEGY in "${STRATEGIES[@]}"; do
  ...
  mkdir -p logs_failure results_failure
  ...
  for ((i=1; i<=NUM_CLIENTS; i++)); do
    python3 client.py "$NUM_SERVERS" "$NUM_CLIENTS" "$i" "$STRATEGY" "$TRIAL" \
      > "logs_failure/client_${STRATEGY}_${i}.txt" 2>&1 &
    client_pids+=($!)
...
(
  ...
  python3 server.py 8001 "content/server1" "$NUM_SERVERS" "$NUM_CLIENTS" "$STRATEGY" 99 \
    > "logs_failure/server_${STRATEGY}_1_restarted.txt" 2>&1 &
  ...
  mv results/*.csv "results_failure/" 2>/dev/null
